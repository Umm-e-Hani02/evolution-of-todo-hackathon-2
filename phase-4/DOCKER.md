...
This directory contains Docker configurations for running the Phase-4 AI-powered todo application.
...
Create a `.env` file in the `phase-4` directory:
...
```bash
cd phase-4/frontend
docker build --build-arg NEXT_PUBLIC_API_URL=http://localhost:8001 -t phase4-frontend:latest .
```
...
|----------|-------------|---------|
| `DATABASE_URL` | Database connection string | `sqlite:///./data/phase4.db` |
...
services:
  ...
  backend:
    ...
    environment:
      DATABASE_URL: postgresql://phase4user:${DB_PASSWORD}@postgres:5432/phase4
...
- Check logs: `docker logs phase4-backend`
...
```bash
# For SQLite
docker cp phase4-backend:/app/data/phase4.db ./backup.db

# For PostgreSQL
docker exec postgres pg_dump -U phase4user phase4 > backup.sql
```
